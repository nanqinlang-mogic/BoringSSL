...
///   static const uint8_t padding[16] = { 0 };  // Padding is all zeros.
  ...
  CRYPTO_poly1305_update(&ctx, ad, ad_len, ciphertext, ciphertext_len);
///   if (ad_len % 16 != 0) {
...
static int seal_impl(aead_poly1305_update poly1305_update,
                     ...
                     const uint8_t *in, size_t in_len, const uint8_t *extra_in, size_t extra_in_len,
...
///              extra_in_len);
    aead_poly1305(poly1305_update, data.out.tag, c20_ctx, nonce, ad, ad_len, out, in_len, out_tag,
             extra_in_len);
  }
...
static int open_impl(aead_poly1305_update poly1305_update,
                     ...
                     const uint8_t nonce[12],
                     const uint8_t *in, size_t in_len, const uint8_t *in_tag, size_t in_tag_len,
                     const uint8_t *ad, size_t ad_len) {
...
///   }
  ...
  if (asm_capable()) {
    ...
    aead_poly1305(poly1305_update, data.out.tag, c20_ctx, nonce, ad, ad_len, in, in_len, NULL, 0);
    CRYPTO_chacha_20(out, in, in_len, c20_ctx->key, nonce, 1);
  }
